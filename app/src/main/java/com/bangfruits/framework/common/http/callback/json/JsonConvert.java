...
import com.bangfruits.framework.common.http.GsonConvert;
...
public class JsonConvert<T> implements Converter<T> {
    ...
    private T parseParameterizedType(Response response, ParameterizedType type) throws Exception {
        ...
        } else if (rawType == ResultJson.class) {
            // code json格式
    ...
    private T convertBaseCodeJson(Response response,ParameterizedType type,JsonReader jsonReader,Type typeArgument) {
        ...
        if (typeArgument == Void.class) {
            // 泛型格式如下： new JsonCallback<ResultJson<Void>>(this)
            ...
            ResultJson baseCodeJson = GsonConvert.fromJson(jsonReader,type);
            response.close();
